  ...
  bindEvents() {
    ...
    if (options.mouseDrag) {
      $panel.on(
        ...
        $$1.proxy(this.onDragStart, this)
      );
      $panel.on(SlidePanel.eventName("dragstart selectstart"), event => {
        /* eslint consistent-return: "off" */
        if (options.mouseDragHandler) {
          if (
            !$$1(event.target).is(options.mouseDragHandler) &&
            !($$1(event.target).parents(options.mouseDragHandler).length > 0)
          ) {
            return;
          }
        }
